...
import { db, plumbingProjectsTable } from "@workspace/db";
...
const defaultMilestones = (total: number) => [
  ...
  { milestone: 2, label: "Rough-In", percentage: 35, amount: total * 0.35, status: "pending", date: null },
  { milestone: 3, label: "Top-Out", percentage: 25, amount: total * 0.25, status: "pending", date: null },
  { milestone: 4, label: "Final", percentage: 15, amount: total * 0.15, status: "pending", date: null },
];

router.get("/plumbing-projects", async (_req, res): Promise<void> => {
  const projects = await db.select().from(plumbingProjectsTable);
  projects.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  res.json(projects.map((p) => cleanProject(p as unknown as Record<string, unknown>)));
});

router.post("/plumbing-projects", async (req, res): Promise<void> => {
  const body = req.body;
  const [project] = await db
    .insert(plumbingProjectsTable)
    .values({
      name: body.name,
      ...
      serviceType: body.serviceType ?? "Bathroom Renovation",
      notes: body.notes ?? "",
      estimateData: {},
...
router.get("/plumbing-projects/:id", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid ID" }); return; }
  const [project] = await db.select().from(plumbingProjectsTable).where(eq(plumbingProjectsTable.id, id));
  if (!project) { res.status(404).json({ error: "Project not found" }); return; }
  res.json(cleanProject(project as unknown as Record<string, unknown>));
});

router.patch("/plumbing-projects/:id", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid ID" }); return; }
  const body = req.body;
  ...
  const [project] = await db
    .update(plumbingProjectsTable)
    .set(updateData)
    .where(eq(plumbingProjectsTable.id, id))
    .returning();
  if (!project) { res.status(404).json({ error: "Project not found" }); return; }
  res.json(cleanProject(project as unknown as Record<string, unknown>));
});

router.delete("/plumbing-projects/:id", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid ID" }); return; }
  const [project] = await db.delete(plumbingProjectsTable).where(eq(plumbingProjectsTable.id, id)).returning();
  if (!project) { res.status(404).json({ error: "Project not found" }); return; }
  res.sendStatus(204);
});

router.post("/plumbing-projects/:id/eyespyr", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid ID" }); return; }
  const [existing] = await db.select().from(plumbingProjectsTable).where(eq(plumbingProjectsTable.id, id));
...
router.post("/plumbing-projects/:id/payment", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid ID" }); return; }
  const [existing] = await db.select().from(plumbingProjectsTable).where(eq(plumbingProjectsTable.id, id));
  if (!existing) { res.status(404).json({ error: "Project not found" }); return; }
...
router.post("/plumbing-projects/:id/truthvault", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid ID" }); return; }
  const [existing] = await db.select().from(plumbingProjectsTable).where(eq(plumbingProjectsTable.id, id));
  if (!existing) { res.status(404).json({ error: "Project not found" }); return; }
