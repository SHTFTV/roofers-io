...
import { db, fabricationProjectsTable } from "@workspace/db";
...
const defaultMilestones = (total: number) => [
  ...
  { milestone: 2, label: "Material & Cut", percentage: 30, amount: total * 0.30, status: "pending", date: null },
  { milestone: 3, label: "Weld & Finish", percentage: 25, amount: total * 0.25, status: "pending", date: null },
  { milestone: 4, label: "Final", percentage: 20, amount: total * 0.20, status: "pending", date: null },
];

router.get("/fabrication-projects", async (_req, res): Promise<void> => {
  const projects = await db.select().from(fabricationProjectsTable);
  projects.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  res.json(projects.map((p) => cleanProject(p as unknown as Record<string, unknown>)));
});

router.post("/fabrication-projects", async (req, res): Promise<void> => {
  const body = req.body;
  const [project] = await db
    .insert(fabricationProjectsTable)
    .values({
      name: body.name,
      ...
      serviceType: body.serviceType ?? "Steel Fabrication",
      notes: body.notes ?? "",
      estimateData: {},
...
router.get("/fabrication-projects/:id", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid ID" }); return; }
  const [project] = await db.select().from(fabricationProjectsTable).where(eq(fabricationProjectsTable.id, id));
  if (!project) { res.status(404).json({ error: "Project not found" }); return; }
  res.json(cleanProject(project as unknown as Record<string, unknown>));
});

router.patch("/fabrication-projects/:id", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid ID" }); return; }
  const body = req.body;
  ...
  const [project] = await db
    .update(fabricationProjectsTable)
    .set(updateData)
    .where(eq(fabricationProjectsTable.id, id))
    .returning();
  if (!project) { res.status(404).json({ error: "Project not found" }); return; }
  res.json(cleanProject(project as unknown as Record<string, unknown>));
});

router.delete("/fabrication-projects/:id", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid ID" }); return; }
  const [project] = await db.delete(fabricationProjectsTable).where(eq(fabricationProjectsTable.id, id)).returning();
  if (!project) { res.status(404).json({ error: "Project not found" }); return; }
  res.sendStatus(204);
});

router.post("/fabrication-projects/:id/eyespyr", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid ID" }); return; }
  const [existing] = await db.select().from(fabricationProjectsTable).where(eq(fabricationProjectsTable.id, id));
...
router.post("/fabrication-projects/:id/payment", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid ID" }); return; }
  const [existing] = await db.select().from(fabricationProjectsTable).where(eq(fabricationProjectsTable.id, id));
  if (!existing) { res.status(404).json({ error: "Project not found" }); return; }
...
router.post("/fabrication-projects/:id/truthvault", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid ID" }); return; }
  const [existing] = await db.select().from(fabricationProjectsTable).where(eq(fabricationProjectsTable.id, id));
  if (!existing) { res.status(404).json({ error: "Project not found" }); return; }
