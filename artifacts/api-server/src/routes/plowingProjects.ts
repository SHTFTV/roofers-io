...
import { db, plowingProjectsTable } from "@workspace/db";
...
const defaultMilestones = (total: number) => [
  { milestone: 1, label: "Deposit", percentage: 30, amount: total * 0.30, status: "pending", date: null },
  { milestone: 2, label: "Mid-Season", percentage: 35, amount: total * 0.35, status: "pending", date: null },
  { milestone: 3, label: "End-Season", percentage: 25, amount: total * 0.25, status: "pending", date: null },
  { milestone: 4, label: "Final", percentage: 10, amount: total * 0.10, status: "pending", date: null },
];

router.get("/plowing-projects", async (_req, res): Promise<void> => {
  const projects = await db.select().from(plowingProjectsTable);
  projects.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  res.json(projects.map((p) => cleanProject(p as unknown as Record<string, unknown>)));
});

router.post("/plowing-projects", async (req, res): Promise<void> => {
  const body = req.body;
  const [project] = await db
    .insert(plowingProjectsTable)
    .values({
      name: body.name,
      ...
      serviceType: body.serviceType ?? "Residential Driveway",
      notes: body.notes ?? "",
      estimateData: {},
...
router.get("/plowing-projects/:id", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid ID" }); return; }
  const [project] = await db.select().from(plowingProjectsTable).where(eq(plowingProjectsTable.id, id));
  if (!project) { res.status(404).json({ error: "Project not found" }); return; }
  res.json(cleanProject(project as unknown as Record<string, unknown>));
});

router.patch("/plowing-projects/:id", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid ID" }); return; }
  const body = req.body;
  ...
  const [project] = await db
    .update(plowingProjectsTable)
    .set(updateData)
    .where(eq(plowingProjectsTable.id, id))
    .returning();
  if (!project) { res.status(404).json({ error: "Project not found" }); return; }
  res.json(cleanProject(project as unknown as Record<string, unknown>));
});

router.delete("/plowing-projects/:id", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid ID" }); return; }
  const [project] = await db.delete(plowingProjectsTable).where(eq(plowingProjectsTable.id, id)).returning();
  if (!project) { res.status(404).json({ error: "Project not found" }); return; }
  res.sendStatus(204);
});

router.post("/plowing-projects/:id/eyespyr", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid ID" }); return; }
  const [existing] = await db.select().from(plowingProjectsTable).where(eq(plowingProjectsTable.id, id));
...
router.post("/plowing-projects/:id/payment", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid ID" }); return; }
  const [existing] = await db.select().from(plowingProjectsTable).where(eq(plowingProjectsTable.id, id));
  if (!existing) { res.status(404).json({ error: "Project not found" }); return; }
...
router.post("/plowing-projects/:id/truthvault", async (req, res): Promise<void> => {
  const id = parseInt(req.params.id);
  if (isNaN(id)) { res.status(400).json({ error: "Invalid ID" }); return; }
  const [existing] = await db.select().from(plowingProjectsTable).where(eq(plowingProjectsTable.id, id));
  if (!existing) { res.status(404).json({ error: "Project not found" }); return; }
